...
All three skills SHALL obtain the current agent session ID from their runner before normal delivery and SHALL rely on the selected message template to include that value in its metadata header. The runner SHALL prefer a non-empty explicit generic session ID, otherwise a non-empty `CLAUDE_CODE_SESSION_ID` supplied by Claude Code, and otherwise a non-empty `CODEX_THREAD_ID` supplied by Codex. It SHALL expose the selected value as generic template runtime metadata and through the legacy Codex compatibility field without confusing it with Discord's `--thread-id` delivery option. If no supported identifier is available, the skill SHALL stop the normal send and invoke bounded failure reporting instead of inventing an identifier.

#### Scenario: Current thread is available
- **WHEN** `CODEX_THREAD_ID` contains a non-empty identifier and no higher-priority session identifier is present
- **THEN** the skill proceeds and the rendered Discord message includes the Codex identifier as `runtime.session.id`
...
- **WHEN** `CLAUDE_CODE_SESSION_ID` contains a non-empty identifier and no explicit generic identifier is present
- **THEN** the skill proceeds and the rendered Discord message includes the Claude Code identifier as `runtime.session.id`
...
- **THEN** the runner selects the Claude Code identifier for the rendered metadata header
...
- **THEN** the runner selects the explicit generic identifier for the rendered metadata header
...
For an enabled automatic notification policy, the agent SHALL establish one concise session name on first activation and reuse it throughout the conversation. Every runner invocation SHALL provide or discover a normalized agent name, project name, session name, and session ID so the selected template can identify the notification source. The runner SHALL infer supported agent and project values when explicit values are absent, and SHALL use `session-<ID prefix>` when no human-readable session name was established.
...
- **THEN** the agent chooses a concise stable name such as `notification-skill-design` and uses it for every notification in that conversation
...
- **THEN** it infers the agent and project and derives a deterministic session name from the session ID
...
**Reason**: One six-state skill conflates notifications sent during work with the single outcome notification required before yielding to the user.

**Migration**: Replace `ping-me-report-agent-status` with `ping-me-report-work-progress` for continuing-work states and `ping-me-report-turn-outcome` for final or waiting states.
