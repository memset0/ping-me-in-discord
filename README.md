...
`[defaults].template = "defaults"` selects `templates/defaults.md`. It may instead contain an absolute path ending in `.md`. A new default template renders the message first, then appends available host, project, session, agent, and local-time context as one Discord subtext footer without an extra blank line. The thread segment prefers the session title and falls back to the full session ID:
