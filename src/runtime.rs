...
impl RuntimeMetadata {
    pub(crate) fn capture() -> Self {
        let now = Zoned::now();
        Self::from_zoned(whoami::username().ok(), whoami::hostname().ok(), &now)
    }

    fn from_zoned(user: Option<String>, hostname: Option<String>, now: &Zoned) -> Self {
    ...
    #[cfg(test)]
    pub(crate) fn fixed(user: &str, hostname: &str, local: &str, unix: i64, iso8601: &str) -> Self {
        Self {
...
mod tests {
    ...
    fn derives_all_timestamp_forms_from_one_zoned_instant() {
        ...
        let runtime =
            RuntimeMetadata::from_zoned(Some("mem".to_owned()), Some("vultr".to_owned()), &now);
